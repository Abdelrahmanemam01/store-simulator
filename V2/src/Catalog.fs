...
// Lazy initialization function - will be set by DatabaseService module at runtime
let mutable private loadCatalogImpl : (unit -> ProductCatalog) option = None

let setLoadCatalogFunction (fn: unit -> ProductCatalog) =
    loadCatalogImpl <- Some fn

let loadCatalog () : ProductCatalog =
    match loadCatalogImpl with
    | Some fn -> 
        // DatabaseService has registered itself, use it
        fn()
    | None -> 
        // DatabaseService not registered yet - return empty catalog
        // Caller should ensure initializeDatabase() is called before loadCatalog()
        Map.empty
